...
export class TemplateHelper {
  ...
  public static renderTemplate(
    ...
    if ((template as any).$parentTemplateContext) {
      context = {
        ...context,
        $parent: (template as any).$parentTemplateContext,
      };
    }
    ...
    if (template.content && template.content.childNodes.length) {
      ...
      for (let i = 0; i < template.childNodes.length; i++) {
        const clonedNode = this.simpleCloneNode(template.childNodes[i]);
        if (clonedNode) {
  ...
  private static expandExpressionsAsString(
    ...
    return str.replace(this.expression, (match, p1) => {
      const value = this.evalInContext(p1 || this.trimExpression(match), {
        ...context,
        ...additionalContext,
      });
        ...
        if (attribute.name === 'data-props') {
          ...
          for (const prop of propsValue.split(',')) {
            ...
            if (keyValue.length === 2) {
              ...
              const value = this.evalInContext(keyValue[1].trim(), {
                ...context,
                ...additionalContext,
              });
              ...
              if (key.startsWith('@')) {
                ...
                if (typeof value === 'function') {
                  nodeElement.addEventListener(key.substring(1), e =>
                    value(e, context, root)
                  );
                }
              } else {
                (nodeElement as any)[key] = value;
              }
            }
          }
        } else {
          const attrValue = this.expandExpressionsAsString(
            attribute.value,
            context,
            additionalContext
          );
          ...
          if (attrValue) {
            nodeElement.setAttribute(attribute.name, attrValue);
          }
        }
      }
    }
    ...
    for (let i = 0; i < node.childNodes.length; i++) {
      ...
      if (childElement.dataset) {
        ...
        if (childElement.dataset.if) {
          ...
          if (
            !this.evalBoolInContext(this.trimExpression(expression), {
              ...context,
              ...additionalContext,
            })
          ) {
    ...
    for (let i = 0; i < loopChildren.length; i++) {
      ...
      if (loopExpression) {
        ...
        if (loopTokens.length === 3) {
          ...
          const list = this.evalInContext(listKey, {
            ...context,
            ...additionalContext,
          });
  ...
  private static evalBoolInContext(expression: any, context: any) {
  ...
  private static evalInContext(expression: any, context: any) {
    context = { ...context, ...this.globalContext };
